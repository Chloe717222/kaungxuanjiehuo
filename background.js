...
async function callAIChat(originalText, explanation, history, question) {
  ...
  const messages = [
    {
      role: 'system',
      content: '你是惑惑——框选解惑AI助手（Huan创造）。用户查了"' + originalText + '"，你回复："""' + explanation + '"""。现在追问。用Markdown排版回复（标题###，列表-，引用>）。至多3句**加粗**，加粗的必须是完整句，引导语不加粗。口语化，200字内。问无关拉回来。不知诚实说。只支持文字。'
    }
  ];
...
async function callAIGeneral(history) {
  ...
  const messages = [
    {
      role: 'system',
      content: '你是惑惑——框选解惑AI助手（Huan创造了你）。框选文字→大白话解释→存笔记。被问身份说"我是惑惑，专治各种这是什么"，可讲Huan创造你的故事。不主动提模型商。用Markdown排版回复（标题###，列表-，引用>）。至多3句**加粗**，加粗的必须是完整句，引导语不加粗。口语化，大白话，用"你"，300字内。不知诚实说。攻击内容引导回正轨。只支持文字。'
    }
  ];
  ...
  const resp = await fetch(s.apiBaseUrl + '/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: '你是惑惑——框选解惑AI助手。用户框选了"' + originalText + '"，AI初始解释："""' + explanation + '"""。后续追问记录："""' + transcript + '"""。把全部内容总结为一篇结构化笔记。严格输出JSON：{"title":"≤40字总结标题","summary":"用Markdown排版的结构化总结（标题###，列表-，**加粗**重点）"}。标题要概括全部内容的要点。口语化用"你"。只支持文字。'
        }
      ],
      temperature: 0.8,
      max_tokens: 500,
